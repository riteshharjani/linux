 ...
 * cache. That will allow ext4_map_blocks_atomic_write() to return the unwritten
 * extent range w/o going into the slow path. That means we might need a loop
 * for conversion of this unwritten extent split across leaf block within a
 * single journal transaction. Split extents across leaf nodes is a rare case,
 * but let's still handle that to meet the requirements of multi-fsblock atomic
 * writes.
 *
