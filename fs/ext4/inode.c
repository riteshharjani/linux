				  ...
				  struct ext4_map_blocks *map)
{
	...
	do {
		...
		if (ret < 0)
			goto out;
		WARN_ON_ONCE(!mixed_mapping && ret < m_len);
		if (ret == m_len)
			goto out;
	...
	ret = ext4_map_blocks(handle, inode, map, 0);
	if (ret != m_len) {
		ext4_warning_inode(inode, "allocation failed for atomic write request pos:%u, len:%u, mixed_mapping:%s\n",
			m_lblk, m_len, mixed_mapping ? "true" : "false");
		ret = -EINVAL;
	}
out:
	return ret;
