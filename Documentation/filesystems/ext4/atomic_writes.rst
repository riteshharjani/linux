...
1. **Single-fsblock Atomic Writes**:
   EXT4's initial implementation supported atomic write operations with a single
   filesystem block. The atomic write unit minimum and maximum sizes are both
   set to filesystem blocksize.
   e.g. doing atomic writes of 16KB filesystem blocksize on 64KB pagesize system
   is possible.
...
flag, require that either all data is written or none at all. In the event of
a system crash or unexpected power loss during the write operation, the affected
region must reflect either the complete old data or the complete new data, but
never a mix of both.
...
mid-write, the system could observe partially updated regions after reboot, new
data over mapped areas, and stale (old) data over unwritten extents that were
never marked written. This violates the atomicity and/or torn write prevention
guarantee.
...
operation performed by iomap, ensuring consistency and correctness.
...
Handling Journaling transactions
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
reserved during:
 ...
 1. Block allocation time in ``ext4_iomap_alloc()``. We first query if there
    ...
    reserve credits of up to ``m_len/2``, assuming every alternate block can be
    an unwritten extent or a hole.
