...
static inline void __init hash_kfence_add_slot(phys_addr_t paddr, int slot)
{
	...
	if (!kfence_pool)
		return;
	VM_BUG_ON(!is_kfence_address((void*)vaddr));
	VM_BUG_ON(lmi >= linear_map_kf_hash_count);
...
static int hash_kfence_map_pages(struct page *page, int numpages, int enable)
{
	...
	VM_BUG_ON(!linear_map_kf_hash_count);
	local_irq_save(flags);
	for (i = 0; i < numpages; i++, page++) {
		vaddr = (unsigned long)page_address(page);
		lmi = (vaddr - (unsigned long)__kfence_pool) >> PAGE_SHIFT;

		/* For kfence this should never happen, right? */
		VM_BUG_ON(lmi >= linear_map_kf_hash_count);
